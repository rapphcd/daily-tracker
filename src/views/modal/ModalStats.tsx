...
function ModalStats() {
	...
	useEffect(() => {
		...
		const calculateMoodStats = () => {
			...
			for (const key of Object.keys(logs).slice(-30)) {
				const log = logs[key];
				if (log == undefined || log.mood == undefined) continue;
				const mood: "1" | "2" | "3" | "4" | "5" = `${log.mood}`;
				stats[mood] += 1;
				stats.days += 1;
			}
			for (const key of Object.keys(stats) as (keyof typeof stats)[]) {
				if (key == "days") continue;
				stats[key] = (stats[key] / stats.days) * 100 | 0;
			}
			setMoodStats(stats);
		}
							...
							<div  style={{
								...
								<div style={{
									...
									<div style={{
										...
										<BarChart
											...
											data={Object.keys(logs).sort((a, b) => new Date(a) > new Date(b) ? 1 : -1).slice(-7).map((k) => {
												...
												if (log.sleep != undefined) {
													return {
														name: `${j}/${m}`,
														sleeptime: Math.round((log.sleep.time / 60) * 100) / 100,
										...
										>
											...
											<XAxis dataKey="name"/>
											<YAxis width="auto"/>
											<Tooltip labelStyle={{color: 'var(--text-normal)'}} contentStyle={{
												backgroundColor: 'var(--background-primary)',
												borderColor: 'var(--background-modifier-border)'
											}}/>
											<Legend/>
											<Bar dataKey="sleeptime" fill="#8884d8"
											     activeBar={{fill: '#8884d8', stroke: 'black'}} onClick={(b) => {
												if (b.name != undefined) setActiveLog(b.name, false);
											}} radius={[3, 3, 0, 0]}/>
											<Bar dataKey="worktime" fill="#82ca9d"
											     activeBar={{fill: '#82ca9d', stroke: 'black'}} onClick={(b) => {
												if (b.name != undefined) setActiveLog(b.name, false);
											}} radius={[3, 3, 0, 0]}/>
										</BarChart>
									</div>
								...
								<div style={{
									...
									<div style={{
										...
										{
											moodStats.days > 0 && (Object.keys(moodStats) as (keyof typeof stats)[]).filter((k) => k != "days").map((k) => (
												<div key={k} style={{
													display: "flex",
													justifyContent: "center",
													alignItems: "center",
								...
								}}>
									{
										data.length >= 3 ? (
											...
											>
												...
												/>
												<PolarRadiusAxis/>
												<Radar name="n" dataKey="B" stroke="#8884d8" fill="#8884d8"
												       fillOpacity={0.6}/>
											</RadarChart>
										) : (
											<div style={{
											...
											Object.keys(logs).filter((a) => logs[a] != undefined && logs[a].sleep != undefined).sort((a, b) => new Date(a) > new Date(b) ? 1 : -1).slice(-4).map((k) => {
												...
												return (
													<div key={log.date} style={{
														...
														<PieChart
															style={{width: '100%', height: 'fit-content', aspectRatio: 1}}
															responsive>
															<Pie
																data={[
																	{
