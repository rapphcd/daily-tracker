...
function TrackingProvider({children, plugin, saveLogs, app}: Props) {
	...
	useEffect(() => {
		if (!logs[todayDate]) {
			...
			setLogs(prev => {
				return {...prev, [todayDate]: {habits: hab, sleep: {time: 0, start: "", end: ""}, summary: "", workTime: 0, date: todayDate, mood: 3}}
			});
		}
	...
	const setActiveLog = (date: string, create: boolean) => {
		if (!logs[date]) {
			...
			const newLog : DailyLog = {habits: hab, sleep: {time: 0, start: "", end: ""}, summary: "", workTime: 0, date: date, mood: 3};
